...
async def select_download_type_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    url = context.user_data.get(f'url_for_download_{user_id}', None)
    if not url:
...
async def select_video_quality_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sel_user_id != user_id:
        await query.edit_message_text("This button is not for you.")
        return

    # Définir le format selon qualité
    format_map = {"360p": "bestvideo[height<=360]+bestaudio/best",
                  "720p": "bestvideo[height<=720]+bestaudio/best",
                  "1080p": "bestvideo[height<=1080]+bestaudio/best"}
    selected_format = format_map.get(quality, "bestvideo+bestaudio/best")

    task = asyncio.create_task(download_video(query, context, url, selected_format))
    context.bot_data.setdefault('active_downloads', {})[user_id] = {'task': task}
...
async def download_audio(query, context, url):
    ...
    try:
        ...
        context.user_data.pop(f'url_for_download_{user_id}', None)

async def download_video(query, context, url, format_str):
    chat_id = query.message.chat_id
    temp_dir = tempfile.mkdtemp()
...
async def handle_search_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not entries:
        await update.message.reply_text(texts.get("no_results", "Nothing found."))
        return
